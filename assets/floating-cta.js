(function () {
  ...
  var CONTACT_URL = "https://www.fypromogifts.com/#contact";
  ...
  function injectStyle() {
    ...
    style.textContent =
      ...
      ".fy-floating-quote{position:fixed!important;z-index:2147483001!important;right:24px;min-height:54px;padding:0 23px;border-radius:999px;gap:10px;background:linear-gradient(180deg,#f4c76f,#d99b38);color:#16110a;font-size:15px;font-weight:900;letter-spacing:.01em;box-shadow:0 16px 34px rgba(217,155,56,.30),0 0 0 7px rgba(217,155,56,.10);animation:fyQuoteGlow 3.2s ease-in-out infinite}" +
      ".fy-floating-quote:hover,.fy-floating-quote:focus-visible{transform:translateY(-1px);background:linear-gradient(180deg,#ffd27a,#d99b38);box-shadow:0 20px 42px rgba(217,155,56,.42),0 0 0 8px rgba(217,155,56,.14)}" +
      ...
      ".fy-floating-quote .fy-floating-tip{left:auto;right:0;transform:translateY(4px)}" +
      ...
      ".fy-floating-quote:hover .fy-floating-tip,.fy-floating-quote:focus-visible .fy-floating-tip{transform:translateY(0)}" +
      ...
      "@media(max-width:720px){.fy-floating-cta a{bottom:calc(16px + env(safe-area-inset-bottom,0px))}.fy-floating-wa{left:16px;width:56px;height:56px}.fy-floating-wa svg{width:29px;height:29px}.fy-floating-quote{right:16px;min-height:48px;padding:0 16px;gap:8px;font-size:13px}.fy-floating-quote svg{width:17px;height:17px}.fy-floating-tip{display:none}}" +
      "@media(prefers-reduced-motion:reduce){.fy-floating-wa,.fy-floating-wa:before,.fy-floating-quote{animation:none}}";
  ...
  function mount() {
    ...
    quote.className = "fy-floating-quote";
    quote.href = isHomePage() ? "#contact" : CONTACT_URL;
    quote.setAttribute("aria-label", "Get a Free Quote");
    quote.innerHTML = "<span>Get a Free Quote</span>" + quoteIcon() + '<span class="fy-floating-tip">Send logo, quantity &amp; deadline</span>';
