...
const curatedProductIds = new Set(['MU024', 'MU023', 'DR004']);
...
${footer()}<script src="/catalog/assets/catalog.js?v=${assetVersion}" defer></script><script src="/assets/fy-attribution.js?v=20260828" defer></script><script src="/assets/fy-inquiry-guard.js?v=20260902" defer></script></body></html>`;

// The gift-set category now contains a curated buyer guide, community answers,
// anonymized production proof and verified procurement facts. Do not replace it
// with the legacy minimal category template during routine product generation.
console.log('Preserved curated catalog/gift-sets/index.html.');

for (const product of detailProducts) {
  // These verified top-seller pages contain confirmed MOQ, lead-time and
  // packaging facts that are intentionally maintained by the SEO workflow.
  if (curatedProductIds.has(product.product_id)) continue;
