...
const assetVersion = '20260812-gift2';
...
for (const product of products) {
  ...
  const schema = {
    '@context': 'https://schema.org',
    '@graph': [
      { '@type': 'Product', '@id': `${url}#product`, name: product.title_en, description: product.short_description, sku: product.product_id, category: 'Custom Corporate Gift Sets', material: product.material, color: product.colors.join(', '), image: images.map(absolute), url, additionalProperty: [
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${esc(product.title_en)} | Custom Gift Set</title><meta name="description" content="${esc(product.short_description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${url}">
